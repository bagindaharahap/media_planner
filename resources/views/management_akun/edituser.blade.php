    ...
    <div @click.outside="showEditModal = false" class="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden border border-slate-200" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0">
        ...
        <div class="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-lg font-bold text-slate-800 tracking-tight">Edit Data Pengguna</h3>
            </div>
        ...
        <!-- Bungkus dengan Form agar Data dapat di Submit ke Laravel -->
        <form :action="`{{ url('/management-akun') }}/${selectedUser?.id}`" method="POST" x-show="selectedUser">
            @csrf
            @method('PUT') <!-- Method Spoofing untuk UPDATE Laravel -->
            ...
            <div class="p-8 space-y-5">
                ...
                <div class="space-y-2">
                    <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Status Akun</label>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input :type="showPassword ? 'text' : 'password'" name="password" placeholder="Kosongkan jika tidak ingin mengubah password" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 pr-12 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <p class="text-[10px] text-slate-400 ml-1">Minimal 8 karakter. Kosongkan jika tidak ingin mengubah password.</p>
                </div>
            </div>

            <!-- Footer -->
            <div class="px-8 py-5 border-t border-slate-100 bg-slate-50/50 flex gap-3">
                <button type="button" @click="showEditModal = false; resetPasswordVisibility()" class="flex-1 py-3 text-sm font-bold text-slate-500 hover:bg-slate-200 rounded-2xl transition-all">Batal</button>
                <button type="submit" class="flex-[2] bg-indigo-600 text-white py-3 rounded-2xl font-bold text-sm hover:bg-indigo-700 transition-all">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>