...
@section('title', 'Account Management - Content Planner')
...
<div 
    x-data="{
        ...
        // User data will be filled from the script tag above during init()
        users: [],
...
>
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 font-medium">Manage access rights, roles, and Content Planner team members.</p>
        </div>
        <button @click="showCreateModal = true; resetPasswordVisibility()" class="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">
    ...
    <div class="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-wrap items-center gap-4">
        <div class="flex-1 min-w-[250px] relative">
            ...
            <input x-model="searchQuery" type="text" placeholder="Search user name or email..." class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all">
        </div>
    ...
    <div class="bg-white border border-slate-100 rounded-[2rem] shadow-sm overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-100">
                        ...
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Access & Role</th>
                ...
                <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-8 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs font-bold" :class="user.status === 'Aktif' ? 'text-slate-700' : 'text-slate-400'" x-text="user.status === 'Aktif' ? 'Active' : 'Inactive'"></span>
                                </div>
                            </td>

                            <td class="px-8 py-4 text-right">
                                <div class="flex items-center justify-end gap-2 transition-opacity">
                                    <button @click="openEdit(user)" class="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all" title="Edit User">
                                        <i class="fa-solid fa-pen text-[10px]"></i>
                                    </button>
                                    <button @click="confirmDelete(user)" class="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center text-slate-400 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-all" title="Delete User">
                    ...
                    <tr x-show="filteredUsers.length === 0">
                        <td colspan="4" class="px-8 py-12 text-center">
                            ...
                            <p class="text-slate-500 font-bold">User not found</p>
                            <p class="text-xs text-slate-400 mt-1">Try using different keywords or the database might be empty.</p>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="px-8 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Showing <span x-text="filteredUsers.length"></span> Users</p>
        </div>
    </div>
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-transition.opacity>
        <div @click.outside="showDeleteModal = false" class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 border border-slate-100 text-center" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0">
            ...
            <h3 class="text-2xl font-bold text-slate-800 mb-2">Delete User?</h3>
            <p class="text-slate-500 text-sm leading-relaxed mb-8 px-4" x-text="selectedUser ? `Are you sure you want to delete access for ${selectedUser.name}?` : ''"></p>
            <div class="flex gap-4">
                <button type="button" @click="showDeleteModal = false" class="flex-1 px-6 py-3.5 rounded-2xl text-sm font-bold text-slate-500 hover:bg-slate-50 border border-slate-100 transition-all">Cancel</button>
                
                <form x-show="selectedUser" :action="`{{ url('/management-akun') }}/${selectedUser?.id}`" method="POST" class="flex-1">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="w-full px-6 py-3.5 bg-rose-500 text-white rounded-2xl font-bold shadow-xl shadow-rose-100 hover:bg-rose-600 transform active:scale-95 transition-all">Yes, Delete</button>
                </form>
            </div>
        </div>
    </div>
