...
<!-- Capture success/error messages from session (fallback) -->
@if(session('success'))
...
<!-- Get original data from Database Controller -->
    ...
    <div class="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 relative z-10">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 font-medium">Manage and save your prompt templates</p>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
            ...
            >
                <i class="fa-solid fa-plus text-xs"></i> Add Prompt
            </button>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-slate-200 text-[10px] font-black uppercase tracking-widest text-slate-400">
                        <th class="p-5 pl-8 rounded-tl-[2rem]">Prompt Title</th>
                        <th class="p-5">Category</th>
                        <th class="p-5">Last Log</th>
                        ...
                        <tr class="hover:bg-indigo-50/30 transition-colors group">
                            ...
                            <td class="p-5">
                                <div class="flex flex-col gap-0.5">
                                    <span class="text-[11px] font-bold text-slate-700">
                                        <span x-text="prompt.log_action || 'Created'"></span> by 
                            ...
                            <td class="p-5 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button @click="openView(prompt)" class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white flex items-center justify-center transition-colors shadow-sm" title="View">
                                    ...
                                    <button @click="deletePrompt(prompt.id)" class="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white flex items-center justify-center transition-colors shadow-sm" title="Delete">
                    ...
                    <template x-if="paginatedPrompts.length === 0">
                        <tr>
                            <td colspan="4" class="p-10 text-center">
                                ...
                                <p class="text-slate-500 font-bold">No prompt data found.</p>
                                <p class="text-xs text-slate-400 mt-1">Try using different keywords or add a new prompt.</p>
                            </td>
                        </tr>
                    </template>
                </tbody>
        ...
        <template x-if="totalPages > 0">
            <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                <p class="text-xs text-slate-500 font-medium">
                    Showing <span class="font-bold text-slate-700" x-text="(currentPage - 1) * itemsPerPage + 1"></span> - 
                    <span class="font-bold text-slate-700" x-text="Math.min(currentPage * itemsPerPage, filteredPrompts.length)"></span> 
                    of <span class="font-bold text-slate-700" x-text="filteredPrompts.length"></span> entries
                </p>
...
function promptNotesData() {
    return {
        ...
        async savePrompt() {
            try {
                if(!this.form.title || !this.form.category) throw new Error("Title and Category are required!");
                
                this.form.log_action = 'Created';
                ...
                if (!res.ok) throw new Error("Failed to save data to database.");
                const data = await res.json();
                
                this.prompts.unshift(data.prompt); 
                
                this.showCreateModal = false;
                AppPopup.success('Success', 'New prompt note has been created.');
            } catch (error) {
                AppPopup.success('Failed', error.message);
            }
        },
        ...
        async updatePrompt() {
            try {
                ...
                AppPopup.success('Successfully Edited', 'Prompt changes have been saved.');
            } catch (error) {
                AppPopup.success('Update Failed', error.message);
            }
        },
        ...
        async deletePrompt(id) {
            AppPopup.confirmDelete(
                'Delete Prompt?',
                'Are you sure you want to delete this prompt note? You will not be able to recover it.',
                async () => {
                    try {
                        ...
                        AppPopup.success('Deleted Successfully', 'Prompt note has been removed from the system.');
                    } catch (error) {
                        AppPopup.success('Delete Failed', error.message);
                    }
                }
            );
        }
