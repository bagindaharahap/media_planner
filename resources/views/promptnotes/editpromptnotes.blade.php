...
>
    ...
    >
        <div class="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-amber-50/30">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 font-medium">Ubah instruksi atau tag pada catatan ini.</p>
                </div>
            </div>
        ...
        <div class="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
            ...
            x-init="$watch('showEditModal', value => { 
                ...
                <div class="border border-slate-200 rounded-3xl overflow-hidden shadow-sm bg-white focus-within:ring-2 focus-within:ring-amber-500/20">
                    ...
                    <!-- Editor Area untuk Edit -->
                    <div x-ref="editEditor" contenteditable="true" @input="form.description = $el.innerHTML" class="editor-content w-full p-6 min-h-[180px] text-sm text-slate-600 focus:outline-none bg-white" data-placeholder="Tuliskan detail prompt di sini..."></div>
                </div>
            </div>
        </div>
        ...
        <div class="px-8 py-6 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-4">
            ...
            <button @click="updatePrompt()" class="bg-amber-500 text-white px-10 py-3 rounded-2xl font-bold text-sm shadow-xl shadow-amber-200 hover:bg-amber-600 transition-all active:scale-95">Simpan Perubahan</button>
        </div>
    </div>
</div>